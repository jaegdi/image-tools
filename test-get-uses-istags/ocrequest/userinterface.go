...
func GetJsonFromAllIstags(list map[string]interface{}, flags T_flags) string {
...
func GetIstagsForFamilyInCluster(flags T_flags, familyNamespaces T_famNs) string {
	...
	if namespace == "" {
		for _, ns := range familyNamespaces[family] {
			r := OcGetAllIstagsOfNamespace(cluster, token, ns)
		...
		result = OcGetAllIstagsOfNamespace(cluster, token, namespace)
	}
	return GetJsonFromAllIstags(result, flags)
}
