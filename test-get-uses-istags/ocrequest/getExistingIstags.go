...
type T_shaStreams map[string]map[string]map[string]string
type T_shaNames map[string]map[string]bool
type T_resIstag map[string]map[string]string

// type T_resSha map[string]map[string]string
type T_resIs map[string]map[string][]string
type T_istag map[string]string
type T_sha map[string]string
type T_isShaTagnames []string
type T_is map[string][]string
type T_resSha map[string]map[string]map[string]string
...
func addNamesToShaStreams(a T_shaStreams, is string, sha string, istag map[string]string) T_shaStreams {
	if a == nil {
		a = T_shaStreams{}
	}
	if a[is] == nil {
...
func appendJoinedNamesToImagestreams(istream T_resIs, imagestreamName string, sha string, ns string, joinedNames []string) T_resIs {
	...
	for _, v := range joinedNames {
		istream[imagestreamName][sha] =
			append(istream[imagestreamName][sha], ns+"/"+v)
	}
	return istream
}

func OcGetAllIstagsOfNamespace(cluster string, token string, namespace string) map[string]interface{} {
	...
	for _, content := range items {
		...
		mySha := map[string]map[string]string{
			imagestreamName: T_sha{
				"name":        joinShaNames(shaNames[sha]),
				"imagestream": imagestreamName,
				"namespace":   isNamespace,
				"link":        isLink,
				"date":        isDate,
		...
		resultIstream = appendJoinedNamesToImagestreams(resultIstream, imagestreamName, sha, isNamespace, joinedNames)
